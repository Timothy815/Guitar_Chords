...
import { computeDrop3Voicings, DROP3_QUALITIES, Drop3Voicing } from './drop3';
import type { Note } from '../../types';

const SET_CONFIG: Record<string, { hex: string; label: string }> = {
  '6-4': { hex: '#f59e0b', label: 'Strings 6–4–3–2' },
  '5-3': { hex: '#14b8a6', label: 'Strings 5–3–2–1' },
};
...
export function Drop3Tab() {
  ...
  const [activeStringSets, setActiveStringSets] = useState<Set<string>>(new Set(['6-4', '5-3']));
  const [activeInversions, setActiveInversions] = useState<Set<string>>(new Set(['inv0', 'inv1', 'inv2', 'inv3']));
  const [addedIndices, setAddedIndices] = useState<Set<number>>(new Set());
  const [showTheory, setShowTheory] = useState(false);

  const quality = DROP3_QUALITIES.find(q => q.key === qualityKey)!;
  const allVoicings = useMemo(() => computeDrop3Voicings(root, quality), [root, quality]);
  ...
  const handlePlay = async (v: Drop3Voicing) => {
    await initAudio();
    const notes = v.frets
  ...
  const sendToProgressions = (v: Drop3Voicing, index: number) => {
    const chord: ChordShape = {
      name: `${root}${quality.label} drop3`,
      frets: v.frets,
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-lg font-serif font-bold text-brand-ink">Drop 3 Voicings</h2>
        <p className="text-sm text-brand-secondary mt-1">
          Four-note chord with the 3rd-highest note dropped an octave — sits across a skipped string, giving a wider, more open sound than Drop 2.
        </p>
      </div>
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-1.5">
          {(['inv0', 'inv1', 'inv2', 'inv3'] as const).map((key, i) => {
            const labels = ['3rd in bass', '5th in bass', '7th in bass', 'Root in bass'];
      ...
      {/* Voicing cards */}
      {voicings.length === 0 ? (
        <p className="text-brand-secondary text-sm">No Drop 3 voicings found for this selection.</p>
      ) : (
          ...
          {voicings.map((v, i) => {
            ...
            return (
              ...
              >
                ...
                {/* Fret map — 4 columns, skip marker for the muted string */}
      ...
      <div className="border border-brand-line rounded-xl overflow-hidden">
        ...
        >
          <span>Drop 2 vs Drop 3 — what's the difference?</span>
        ...
        {showTheory && (
          <div className="px-4 pb-4 pt-3 space-y-2 text-sm text-brand-secondary border-t border-brand-line">
            <p>Both start from the same close-position four-note chord (R–3–5–7). Drop 2 lowers the 2nd-highest note; Drop 3 lowers the 3rd-highest. The key consequence: Drop 3 voicings span five strings but only play four of them — one string is always skipped.</p>
            <p>This wider voicing has a more open, spread sound. It also means the two string sets here ({'"'}6–4–3–2{'"'} and {'"'}5–3–2–1{'"'}) each skip one string in the middle, so the voicing is less compact than Drop 2.</p>
            <p>Drop 3 is used less often than Drop 2 in standard jazz comping, but it appears frequently in solo guitar arrangements and orchestral writing because the wider spacing projects each voice more clearly. The bass note can be plucked with the thumb while the upper three strings are strummed.</p>
            <p><strong className="text-brand-ink">Comparing inversions:</strong> "3rd in bass" (inv0) and "5th in bass" (inv1) are the most common. In root position (inv3) you get the root on the lowest played string with the 7th close underneath the upper notes — useful for a strong harmonic statement.</p>
          </div>
        )}
      </div>
    </div>
